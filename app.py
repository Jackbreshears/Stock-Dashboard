...
def render_sidebar():
    with st.sidebar:
        ...
        for label, vid in [("⚡ Dashboard","dashboard"),("💼 Portfolio","portfolio"),
                           ("👁 Watchlist","watchlist"),("⚙️ Settings","settings")]:
...
def render_dashboard():
    ...
    if port:
        ...
        # ── AI Analyze button ────────────────────────────────────────────
        st.markdown("<br>", unsafe_allow_html=True)
        _ab1, _ab2, _ab3 = st.columns([1, 2, 1])
        with _ab2:
        ...
        if result.get("error"):
            ...
            if gaps:
                ...
                for gap in gaps:
                    ...
                    st.markdown(f"""
                    <div style="background:#0f1520;border:1px solid #1a2540;border-left:3px solid #6366f1;border-radius:0 10px 10px 0;padding:12px 16px;margin-bottom:8px;">
                      <div style="font-size:14px;font-weight:700;color:#818cf8;margin-bottom:3px;">{label}</div>
                      <div style="font-size:13px;color:#64748b;line-height:1.5;">{detail}</div>
                    </div>""", unsafe_allow_html=True)
