...
_defaults = {"ticker":None, "view":"dashboard", "ai_result":None, "ai_ticker":None,
             ...
             "_sidebar_prices": {}, "_sidebar_ts": 0.0}
...
def fetch_fast_prices(tickers_key: str) -> dict:
    """Bulk-fetch prices + 52w range using fast_info."""
    result = {}
    for sym in tickers_key.split(","):
        sym = sym.strip()
        if not sym: continue
        try:
...
def fetch_spy_history(period: str) -> dict:
    try:
...
def log_trade(ticker: str, ttype: str, shares: float, price: float,
    ...
    trade = {"id": str(uuid.uuid4())[:8], "ticker": ticker.upper(),
...
def render_sidebar():
    with st.sidebar:
        ...
        if port:
            ...
            if now - st.session_state._sidebar_ts > 300 or not st.session_state._sidebar_prices:
                tkey = ",".join(sorted(port.keys()))
                with st.spinner(""):
                    st.session_state._sidebar_prices = fetch_fast_prices(tkey)
                st.session_state._sidebar_ts = now
            prices = st.session_state._sidebar_prices
...
def render_dashboard():
    ...
    if port:
        ...
        extra_syms  = [s for s in alerts_dict if s not in port]
        all_prices  = prices.copy()
        if extra_syms:
            extra_px = fetch_fast_prices(",".join(sorted(extra_syms)))
            all_prices.update(extra_px)
        triggered = check_alerts(all_prices)
        if triggered:
            for t in triggered:
                col_  = "#34d399" if t["type"] == "above" else "#f87171"
                icon_ = "📈" if t["type"] == "above" else "📉"
                dirn_ = "crossed above" if t["type"] == "above" else "dropped below"
                st.markdown(f"""
                <div style="background:#0f1520;border:1px solid {col_};border-radius:10px;
                  ...
                  <div>
                    ...
                    <span style="color:#94a3b8;margin-left:8px;">{dirn_} your
                      ${t['target']:.2f} alert · now ${t['price']:.2f}</span>
                  </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        for i, r in enumerate(rows_sorted):
            with hold_cols[i % 4]:
                ...
                if st.button(f"{r['sym']}  {arr(r['chg_p'])}{abs(r['chg_p']):.2f}%",
                    ...
                    st.session_state.ticker = r["sym"]; st.session_state.view = "stock"
                    st.session_state.ai_result = None; st.rerun()
                st.markdown(f"""
    ...
    if wl:
        ...
        wtkey = ",".join(sorted(wl.keys()))
        wl_prices = fetch_fast_prices(wtkey)
...
def render_portfolio():
    ...
    tab_h, tab_t, tab_div, tab_al = st.tabs(["Holdings", "Trade Log", "Dividends", "🔔 Alerts"])

    # ── HOLDINGS ──────────────────────────────────────────────────────────
    with tab_h:
        with st.expander("➕ Add / Edit Holding manually", expanded=len(P())==0):
            a1,a2,a3 = st.columns(3)
            with a1: add_sym = st.text_input("Ticker", placeholder="AAPL", key="ph_sym").strip().upper()
            with a2: add_sh  = st.number_input("Shares", min_value=0.001, value=1.0, step=0.5, format="%.3f", key="ph_sh")
            with a3: add_ac  = st.number_input("Avg Cost ($)", min_value=0.01, value=100.0, step=1.0, key="ph_ac")
            add_nt = st.text_area("Notes / thesis", height=60, key="ph_nt")
            if st.button("Save", type="primary", use_container_width=True, key="ph_save"):
                if add_sym:
                    D().setdefault("portfolio",{})[add_sym] = {
                        "shares":add_sh,"avg_cost":add_ac,"notes":add_nt,
                        "added":datetime.now().strftime("%Y-%m-%d")}
                    persist(); st.success(f"Saved {add_sym}"); st.rerun()
        ...
        for r in rows:
            ...
            divs = get_dividends_total(r["sym"])
            ...
            if r["notes"]:
                st.markdown(f"<div style='font-size:12px;color:#475569;border-left:2px solid #1a2540;padding:5px 10px;margin:3px 0;border-radius:0 6px 6px 0;background:#0a0e1a;'>📝 {r['notes']}</div>", unsafe_allow_html=True)
        ...
        with st.spinner("Fetching sector data…"):
            ...
            for sym in port:
                ...
                s_    = info_.get("sector","Other") if not info_.get("_error") else "Other"
                wt_   = (prices.get(sym,{}).get("price",0) * port[sym].get("shares",0) / tv * 100) if tv else 0
                sectors[s_] = sectors.get(s_,0) + wt_
    ...
    with tab_t:
        ...
        with t1: tsym  = st.text_input("Ticker", placeholder="AAPL", key="tl_sym").strip().upper()
        with t2: ttype = st.selectbox("Type", ["buy","sell"], key="tl_type")
        with t3: tsh   = st.number_input("Shares", min_value=0.001, value=1.0, step=0.5, format="%.3f", key="tl_sh")
        with t4: tpr   = st.number_input("Price ($)", min_value=0.01, value=100.0, step=0.5, key="tl_pr")
        with t5: tdt   = st.date_input("Date", value=datetime.now().date(), key="tl_dt")
        tnt = st.text_input("Notes (optional)", key="tl_nt")
        if st.button("Log Trade", type="primary", use_container_width=True, key="tl_log"):
            if tsym:
                log_trade(tsym, ttype, tsh, tpr, str(tdt), tnt)
            ...
            if uploaded:
                try:
                    ...
                    for err in import_errors[:5]:
                        st.warning(err)
                    if preview_rows:
                        st.markdown(f"**{len(preview_rows)} valid trades found:**")
                        ...
                        if st.button(f"✅ Import {len(preview_rows)} trades", type="primary", key="do_import"):
                            for r in preview_rows:
                                log_trade(r["ticker"], r["type"], r["shares"], r["price"], r["date"], r["notes"])
                            st.success(f"Imported {len(preview_rows)} trades!"); st.rerun()
                except Exception as e:
        ...
        if trades:
            ...
            for tr in trades[:50]:
                ...
                """, unsafe_allow_html=True)
                if st.button("×", key=f"del_tr_{tr.get('id','')}", help="Delete this trade"):
                    D()["trades"] = [t for t in D()["trades"] if t.get("id") != tr.get("id")]
                    persist(); st.rerun()
            ...
            if sell_trades:
                ...
                for yr in sorted(by_year.keys(), reverse=True):
                    ...
                    for t in yr_trades:
                        ...
                        buy_dates = [b.get("date","") for b in T()
                                     if b.get("ticker","").upper()==sym_ and b.get("type")=="buy"
                                     and b.get("date","") <= sell_date]
    ...
    with tab_div:
        ...
        with d1: dsym = st.text_input("Ticker", placeholder="AAPL", key="dv_sym").strip().upper()
        with d2: damt = st.number_input("Total $ Received", min_value=0.01, value=10.0, step=0.5, key="dv_amt")
        with d3: ddt  = st.date_input("Date Received", value=datetime.now().date(), key="dv_dt")
        with d4: dnt  = st.text_input("Notes", key="dv_nt")
        if st.button("Log Dividend", type="primary", use_container_width=True, key="dv_log"):
            if dsym:
                ...
                persist(); st.success(f"✅ Logged ${damt:.2f} dividend for {dsym}"); st.rerun()
        ...
        if divs:
            ...
            for sym, dlist in sorted(by_ticker.items()):
                ...
                """, unsafe_allow_html=True)
                for d in dlist:
                    st.markdown(f"<div style='font-size:12px;color:#475569;padding:3px 8px;'>{d.get('date','')} · ${d['amount']:.2f}{' · ' + d['notes'] if d.get('notes') else ''}</div>", unsafe_allow_html=True)
        else:
    ...
    with tab_al:
        ...
        st.caption("Triggered alerts appear as banners on the Dashboard when the price crosses your threshold.")
        ...
        if alerts_dict:
            ...
            for sym, a in list(alerts_dict.items()):
                ...
                with c1:
                    h_tag = f' <span style="color:#34d399;font-size:10px;font-weight:700;">✅ TRIGGERED</span>' if h_triggered else ""
                    l_tag = f' <span style="color:#f87171;font-size:10px;font-weight:700;">✅ TRIGGERED</span>' if l_triggered else ""
                    high_html = f'<span>📈 Above <b style="color:{h_c}">${high:.2f}</b>{h_tag}</span>' if high else ""
                    low_html  = f'<span>📉 Below <b style="color:{l_c}">${low:.2f}</b>{l_tag}</span>'  if low  else ""
                    sep_html  = ' &nbsp;·&nbsp; ' if high and low else ""
                    st.markdown(f"""
                    <div class="card-sm">
                      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
...
def render_watchlist():
    ...
    with st.expander("➕ Add to Watchlist", expanded=len(W())==0):
        w1,w2 = st.columns(2)
    ...
    for sym, meta in list(wl.items()):
        ...
        if notes:
            st.markdown(f"<div style='font-size:12px;color:#475569;border-left:2px solid #1a2540;padding:5px 10px;margin:3px 0;background:#0a0e1a;border-radius:0 6px 6px 0;'>📝 {notes}</div>", unsafe_allow_html=True)
...
def render_stock(ticker: str):
    ...
    with h2:
        ...
        if ticker not in port_:
            if st.button("＋ Portfolio", use_container_width=True, type="primary", key="stk_ap"):
                D().setdefault("portfolio",{})[ticker]={"shares":1.0,"avg_cost":pd_["price"],
                    "notes":"","added":datetime.now().strftime("%Y-%m-%d")}
                persist(); st.success(f"Added {ticker}. Edit shares in Portfolio tab."); st.rerun()
        else:
    ...
    with tab_ov:
        ...
        for i in range(0, len(build_stat_cards(pd_)), 3):
            chunk = build_stat_cards(pd_)[i:i+3]
            cols  = st.columns(3)
    ...
    with tab_ch:
        ...
        for i,(pc_v,pl) in enumerate(periods):
            with pcols[i]:
                active = st.session_state.chart_period == pc_v
                if st.button(pl, key=f"per_{pc_v}", type="primary" if active else "secondary", use_container_width=True):
                    st.session_state.chart_period = pc_v; st.rerun()
