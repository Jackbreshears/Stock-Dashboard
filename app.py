...
.sig-item {
    ...
    font-size: 12px; padding: 7px 10px; margin-bottom: 4px; color: #e6edf3;
}
...
#  Do NOT pass a custom session to yf.Ticker — newer yfinance requires
#  curl_cffi internally and rejects a plain requests.Session.
# ══════════════════════════════════════════════════════════════════════════════
...
def fetch_news(ticker: str) -> list:
    """
    Handles both the old and new yfinance news response formats.
    Old format: flat dict with 'title', 'link', 'publisher', 'providerPublishTime'
    New format: nested under 'content' with 'title', 'canonicalUrl', 'provider', 'pubDate'
    """
    try:
...
def build_chart(ticker: str, period: str):
    ...
    fig = make_subplots(rows=2 if has_vol else 1, cols=1,
                        shared_xaxes=True, row_heights=[0.76, 0.24] if has_vol else [1],
    ...
    if has_vol:
        ...
        fig.add_trace(go.Bar(
            x=dates, y=volumes, marker_color=vol_colors, marker_opacity=0.45,
            name="Volume", hovertemplate="%{y:,.0f}<extra>Volume</extra>",
        ), row=2, col=1)
...
def claude_pros_cons(ticker: str) -> str:
    try:
        ...
        client  = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)
        ctx = (
            f"Stock: {ticker} ({price.get('name', ticker)})\n"
...
def render_analyst(analyst: dict) -> None:
    ...
    if tm and cp:
        ...
        arrow = "▲" if pct >= 0 else "▼"
        st.markdown(
            f'<div class="upside-block"><div class="c-sm">Potential Upside</div>'
    ...
    if tl and th and tm:
        for lbl, val in [("🎯 Average target", f"${tm:.2f}"),
            ...
            st.markdown(f'<div class="t-row"><span class="t-lbl">{lbl}</span>'
                        f'<span class="t-val">{val}</span></div>', unsafe_allow_html=True)
...
def render_pros_cons(text: str) -> None:
    ...
    with c1:
        ...
        st.markdown(f'<div class="pros-box"><div class="pc-hdr" style="color:#3fb950;">✅&nbsp; Reasons to consider buying</div>{items}</div>', unsafe_allow_html=True)
    with c2:
        items = "".join(f'<div class="pc-item">• {it}</div>' for it in cons) or '<div class="pc-item">—</div>'
        st.markdown(f'<div class="cons-box"><div class="pc-hdr" style="color:#f85149;">⚠️&nbsp; Risks to be aware of</div>{items}</div>', unsafe_allow_html=True)
...
def render_news(summary: str | None, headlines: list) -> None:
    ...
    if summary:
        st.markdown(
            f'<div class="ai-sum"><div class="ai-lbl">🤖&nbsp; AI Summary</div>'
...
def main() -> None:
    ...
    with col_in:
        ticker_raw = st.text_input("Ticker", placeholder="e.g. AAPL, TSLA, NVDA, MSFT, AMZN…",
    ...
    with st.container(border=True):
        ...
        period_map = {"1W": "1wk", "1M": "1mo", "3M": "3mo", "6M": "6mo", "1Y": "1y", "5Y": "5y"}
