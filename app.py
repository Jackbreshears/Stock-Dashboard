...
_defaults = {"ticker":None, "view":"dashboard", "ai_result":None, "ai_ticker":None,
             ...
             "_ph_key": 0, "port_ai_result": None, "port_ai_ts": 0.0}
...
def portfolio_ai_analysis(holdings: list, sectors: dict, total_val: float,
    ...
    try:
        ...
        # Build holdings summary (top 10 by value)
        sorted_h = sorted(holdings, key=lambda x: x["cur_val"], reverse=True)[:10]
        holdings_txt = "\n".join(
...
OUTLOOK: [3-sentence growth outlook for this specific portfolio]"""
        ...
        out = {"score":"","health":"","recs":[],"gaps":[],"risk":"","outlook":"","error":None}
        ...
        if not port_az:
            ...
            with st.spinner("Fetching sector data…"):
                ...
                for sym in port_az:
                    info_ = fetch_info(sym)
                    s_    = info_.get("sector", "Other") if not info_.get("_error") else "Other"
                    wt_   = (prices_az.get(sym, {}).get("price", 0) * port_az[sym].get("shares", 0) / tv_az * 100) if tv_az else 0
                    sectors_az[s_] = sectors_az.get(s_, 0) + wt_
