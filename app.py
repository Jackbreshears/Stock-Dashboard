...
def parse_news_item(item):
    ...
    if "content" in item and isinstance(item["content"], dict):
        ...
        try:
            ...
            ts = int(dparse.parse(ts_raw).timestamp()) if ts_raw else 0
        except:
            ts = 0
    else:
        # Old format
...
def similar_stocks(sector):
    ...
    return m.get(sector,[("SPY","S&P 500 ETF"),("QQQ","Nasdaq ETF"),("VTI","Total Market ETF"),("IVV","iShares S&P 500"),("VT","Vanguard Total World")])
...
def ai_quick_take(ticker,info,news_titles):
    ...
    try:
        r=client.messages.create(model="claude-opus-4-5",max_tokens=300,messages=[{"role":"user","content":
            ...
            f"Casual, insightful, no jargon. Start with the company name."}])
...
def ai_pros_cons(ticker,info):
    ...
    try:
        r=client.messages.create(model="claude-opus-4-5",max_tokens=400,messages=[{"role":"user","content":
            ...
            f"Specific, plain English. Format:\nPRO: ...\nPRO: ...\nPRO: ...\nCON: ...\nCON: ...\nCON: ..."}])
        return r.content[0].text.strip(),None
    except Exception as e: return None,str(e)
...
def ai_why_moving(ticker,info,news_titles):
    ...
    try:
        r=client.messages.create(model="claude-opus-4-5",max_tokens=150,messages=[{"role":"user","content":
            ...
            f"Headlines: {'; '.join(news_titles[:5])}. "
            f"ONE plain-English sentence (max 30 words) explaining why. "
            f"Start with the reason, not the company name. Casual language."}])
        return r.content[0].text.strip(),None
    except Exception as e: return None,str(e)
...
def ai_stock_pick():
    ...
    try:
        r=client.messages.create(model="claude-opus-4-5",max_tokens=600,messages=[{"role":"user","content":
            ...
            f"CATALYST1: <catalyst>\nCATALYST2: <catalyst>\nCATALYST3: <catalyst>\n"
            f"RISK1: <key risk>\nRISK2: <key risk>\n"
            f"IDEAL_FOR: <type of investor>"}])
        return r.content[0].text.strip(),None
    except Exception as e: return None,str(e)
...
def ai_portfolio_analysis(tw,info_map,score):
    ...
    try:
        r=client.messages.create(model="claude-opus-4-5",max_tokens=900,messages=[{"role":"user","content":
            ...
            f"For each holding: KEEP or REDUCE. Suggest 2 ADD picks.\n"
...
def html_score_gauge(score):
    ...
    w=score*10
    return (
        '<div style="background:#1e2438;border:1px solid #2e3650;border-radius:12px;padding:20px;text-align:center">'
        ...
        f'<div style="color:{color};font-size:14px;font-weight:600;margin-top:4px">/10 · {label}</div>'
        f'<div style="background:#242b3d;border-radius:20px;height:8px;margin-top:14px;overflow:hidden">'
        f'<div style="width:{w}%;height:100%;background:{color};border-radius:20px"></div></div>'
        '</div>'
    )
...
def html_risk_gauge(risk,label,color,desc):
    ...
    return (
        ...
        f'<span style="font-size:22px;letter-spacing:3px;color:{color}">{filled}</span>'
        f'<span style="font-size:22px;letter-spacing:3px;color:#2e3650">{unfilled}</span>'
        f'<span style="color:{color};font-weight:700;font-size:16px">{label}</span></div>'
        f'<div style="color:#64748b;font-size:12px;line-height:1.4">{desc}</div>'
        '</div>'
    )
...
def html_spy_comparison(ticker,s_pct,m_pct,period_label):
    ...
    return (
        ...
        f'<div style="background:#242b3d;border-radius:8px;padding:10px 12px;'
        f'color:{dc};font-weight:600;font-size:13px;text-align:center">{verdict}</div>'
        '</div>'
    )
...
def html_stock_overview(ticker,info):
    ...
    return (
        ...
        f'<div style="display:flex;flex-wrap:wrap;gap:20px">{rev_block}{rec_block}</div>'
        '</div>'
    )
...
def html_news_card(item):
    ...
    pub_html=(f'<span style="color:{pub_color};font-size:11px;font-weight:700;background:{pub_color}22;'
              f'padding:2px 8px;border-radius:20px">{pub}</span>' if pub else "")
    return (
        f'<a href="{link}" target="_blank" style="text-decoration:none">'
        '<div style="background:#1e2438;border:1px solid #2e3650;border-radius:12px;'
        'padding:16px 18px;margin-bottom:10px;cursor:pointer;display:block">'
        ...
        f'padding:2px 8px;border-radius:20px">{s_icon} {s_label}</span>'
...
def html_portfolio_full(analysis,tw,info_map,score):
    ...
    cfg={"KEEP":("#0f2d1a","#34d399","#166534","✅"),"REDUCE":("#2d0f0f","#f87171","#7f1d1d","⚠️")}
    cards=""
    for h in holdings:
        t=h.get("HOLDING",""); action=h.get("ACTION","KEEP").upper(); reason=h.get("REASON","")
        info=info_map.get(t,{}); price=info.get("currentPrice") or info.get("regularMarketPrice") or 0
        shares=tw.get(t,0); val=price*shares
        bg,border,tag_bg,icon=cfg.get(action,("#1e2438","#94a3b8","#374151","ℹ️"))
        cards+=(
            f'<div style="background:{bg};border:1px solid {border};border-radius:12px;padding:16px;margin-bottom:10px">'
            ...
            f'<div style="color:#e2e8f0;font-weight:600">${price:,.2f} × {shares:.0f}</div>'
    ...
    add_cards=""
    for a in adds:
        sym=a.get("ADD",""); name=a.get("NAME",sym); why=a.get("WHY","")
        add_cards+=(
            f'<div style="background:#0f1f3d;border:1px solid #3b5bdb;border-radius:12px;padding:16px;margin-bottom:10px">'
            f'<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">'
            f'<span style="color:#e2e8f0;font-weight:700;font-size:16px">{sym}</span>'
            f'<span style="color:#7c3aed;font-size:13px">{name}</span>'
            f'<span style="background:#1e3a8a;color:#93c5fd;font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;margin-left:auto">➕ ADD</span></div>'
    ...
    add_section=f'<div style="margin:16px 0 6px;color:#94a3b8;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Suggested Additions</div>{add_cards}' if add_cards else ""
    return (
        f'<div style="background:#1a1040;border:1px solid #6366f1;border-radius:14px;padding:20px;margin-bottom:18px">'
        ...
        f'<div style="color:{score_c};font-size:12px">/100 · {score_l}</div></div>'
        f'<div style="text-align:center;background:#242b3d;border-radius:10px;padding:12px 20px">'
        ...
        f'<div style="margin-bottom:6px;color:#94a3b8;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Current Holdings</div>'
        f'{cards}{add_section}'
        '<div style="margin-top:16px;background:#1e2438;border-radius:8px;padding:10px 14px;color:#475569;font-size:11px">'
        '⚠️ Not financial advice. Always do your own research before investing.</div>'
    )
...
def render_sidebar():
    with st.sidebar:
        ...
        if wl:
            ...
            st.caption("No stocks saved. Tap ⭐ on any stock page.")
        st.markdown("---")
...
GLOSSARY=[
    ...
    ("📊 P/E Ratio","Price-to-Earnings. If P/E is 20, you're paying $20 for every $1 the company earns. Lower = potentially cheaper."),
...
def render_stock_page(ticker):
    ...
    with c1:
        st.markdown(f'<div style="padding:8px 0"><div style="font-size:26px;font-weight:800;color:#e2e8f0">{name}</div>'
                    f'<div style="color:#94a3b8;font-size:14px">{ticker} · {info.get("exchange","")}</div></div>',
                    unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div style="padding:8px 0"><div style="font-size:34px;font-weight:800;color:#e2e8f0">${price:,.2f}</div>'
                    f'<div style="color:{color};font-size:16px;font-weight:600">{arrow} ${abs(chg):.2f} ({chg_pct:+.2f}%)</div></div>',
                    unsafe_allow_html=True)
    with c3:
    ...
    # Watchlist confirmation banner
    if st.session_state.wl_msg:
        msg=st.session_state.wl_msg
    ...
    with cspy:
        period_key=st.selectbox("Compare period",["1y","6mo","3mo","1mo"],index=0,key="spy_period",label_visibility="visible")
    ...
    with r5: st.markdown(html_stat_card("Beta",beta,"1.0 = moves with market. >1 = more volatile"),unsafe_allow_html=True)
    r6,r7,r8,r9,r10=st.columns(5)
    with r6:  st.markdown(html_stat_card("52-Week High",wk52h,"Highest price in past year"),unsafe_allow_html=True)
    with r7:  st.markdown(html_stat_card("52-Week Low",wk52l,"Lowest price in past year"),unsafe_allow_html=True)
    with r8:  st.markdown(html_stat_card("EPS",eps,"Profit divided by shares outstanding"),unsafe_allow_html=True)
    with r9:  st.markdown(html_stat_card("Volume",vol,"Shares traded today"),unsafe_allow_html=True)
    with r10: st.markdown(html_stat_card("Profit Margin",pm,"% of revenue kept as profit"),unsafe_allow_html=True)

    st.markdown("")

    # Price history + calculator
    ...
    with col_calc:
        with st.container(border=True):
            ...
            calc_period=st.radio("Period",["1mo","3mo","6mo","1y","2y","5y"],index=3,horizontal=False,key="calc_period")
            if st.button("Calculate",key="calc_btn"):
                ...
                if hist is not None and len(hist)>1:
                    ...
                    st.markdown(
                        ...
                        f'<div style="color:#475569;font-size:11px;margin-top:6px">Starting from ${invest_amt:,}</div></div>',
                        unsafe_allow_html=True)
                else:
    ...
    with col_chart:
        ...
        if hist is not None and len(hist)>1:
            ...
            lc="#34d399" if float(closes.iloc[-1])>=first else "#f87171"
            fill_c=lc.replace("#","rgba(").replace("d399","211,153,").replace("f87171","248,113,113,")+",0.08)"
            if fill_c==lc: fill_c="rgba(99,102,241,0.08)"
            fig=go.Figure()
    ...
    if news_items:
        ...
        for item in news_items[:10]:
            if item.get("title"):
                st.markdown(html_news_card(item),unsafe_allow_html=True)
                displayed+=1
        if displayed==0:
            st.info("No news headlines available right now.")
    else:
    ...
    for col,(sym,lbl) in zip(sim_cols,sims):
        with col:
            if st.button(f"{sym}",key=f"sim_{sym}",help=lbl,use_container_width=True):
                st.session_state.ticker=sym; st.session_state.company_name=sym; st.rerun()
...
def render_home():
    ...
    with st.container(border=True):
        ...
        with sc:
            q=st.text_input("Search",placeholder="Search — name or ticker (e.g. Apple or AAPL)",
            ...
            if movers:
                ...
                for i,m in enumerate(movers):
                    with cols[i%4]:
                        ...
                        st.markdown(
                            f'<div style="background:#1e2438;border:1px solid #2e3650;border-radius:12px;'
                            ...
                            f'<div style="color:{c};font-weight:600;font-size:15px">{a} {abs(m["chg"]):.2f}%</div></div>',
                            unsafe_allow_html=True)
                        if st.button("View",key=f"mover_{m['ticker']}",use_container_width=True):
                            st.session_state.ticker=m["ticker"]; st.rerun()
            else:
                st.warning("Could not load market data. Please try again.")
        else:
            st.markdown("""
    ...
    with tab2:
        ...
        if pick:
            ...
            st.markdown(
                ...
                f'border:1px solid #6366f1;border-radius:16px;padding:28px;margin-bottom:18px">'
                ...
                f'<span style="background:#06b6d422;color:#06b6d4;font-weight:600;padding:6px 16px;border-radius:20px;font-size:13px">{t_horiz}</span>'
                ...
                f'<div style="background:#242b3d;border-radius:10px;padding:12px">'
                f'<div style="color:#64748b;font-size:11px;font-weight:600;margin-bottom:6px">CATALYSTS</div>{cats}</div>'
                f'<div style="background:#242b3d;border-radius:10px;padding:12px">'
                f'<div style="color:#64748b;font-size:11px;font-weight:600;margin-bottom:6px">KEY RISKS</div>{risks}</div>'
                f'<div style="background:#242b3d;border-radius:10px;padding:12px">'
                f'<div style="color:#64748b;font-size:11px;font-weight:600;margin-bottom:6px">IDEAL FOR</div>'
                f'<div style="color:#e2e8f0;font-size:13px">{t_ideal}</div></div></div></div>',
                unsafe_allow_html=True)
            if t_sym:
                if st.button(f"🔍 Analyze {t_sym}",key="pick_analyze"):
                    st.session_state.ticker=t_sym; st.rerun()
            st.caption("⚠️ Not financial advice. AI-generated content for educational purposes only.")
        else:
            st.markdown("""
    ...
    with tab3:
        ...
        <div style="background:#1e2438;border-left:4px solid #6366f1;border-radius:0 10px 10px 0;padding:14px 18px;margin-bottom:18px">
          ...
          <div style="color:#94a3b8;font-size:13px;line-height:1.6">
            Enter your stock tickers + share counts. We\'ll calculate your diversity score and tell you what to
            <span style="color:#34d399;font-weight:600">KEEP</span>,
            what to <span style="color:#f87171;font-weight:600">REDUCE</span>, and
            what to <span style="color:#93c5fd;font-weight:600">ADD</span> to balance things out.
          </div>
        </div>""",unsafe_allow_html=True)
        with st.container(border=True):
            ...
            if tw:
                with st.spinner("Fetching data and running analysis…"):
                    ...
                    for t,shares in tw.items():
                        ...
                        val=price*shares; total_val+=val
                        sec=inf.get("sector","Unknown"); sectors[sec]=sectors.get(sec,0)+val
    ...
    with tab4:
        ...
        for term,defn in GLOSSARY:
            with st.expander(term):
                st.markdown(f'<div style="color:#cbd5e1;font-size:14px;line-height:1.7;padding:4px 0">{defn}</div>',unsafe_allow_html=True)
        st.markdown("---")
        st.markdown("#### 🎯 Quick Quiz")
        with st.container(border=True):
            st.markdown("**What does a P/E ratio of 30 mean?**")
            a1=st.radio("q1",["The stock has 30% returns","You pay $30 for every $1 the company earns","The company has 30 years of history","The stock dropped 30% this year"],key="quiz_pe",index=None,label_visibility="collapsed")
            if a1:
                if "pay $30" in a1: st.success("✅ Correct! P/E = price you pay per $1 of earnings.")
                else: st.error("❌ P/E = Price ÷ Earnings Per Share. P/E of 30 means you pay $30 per $1 earned.")
        with st.container(border=True):
            st.markdown("**Which best reduces investment risk?**")
            a2=st.radio("q2",["Putting all money in one high-growth stock","Buying only tech stocks","Spreading money across different sectors and assets","Timing the market perfectly"],key="quiz_div",index=None,label_visibility="collapsed")
            if a2:
                if "spreading" in a2.lower(): st.success("✅ Correct! Diversification is the #1 way to reduce risk.")
                else: st.error("❌ Diversification — spreading across sectors — is the answer. Don't put all eggs in one basket!")
