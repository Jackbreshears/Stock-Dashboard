...
.stApp::before{content:'';position:fixed;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,#6366f1,#818cf8,#a78bfa);z-index:9999;}
...
div[data-testid="stVerticalBlockBorderWrapper"]{background:var(--bg2)!important;border:1px solid var(--border)!important;border-radius:16px!important;box-shadow:0 4px 24px rgba(0,0,0,0.22)!important;}

/* Tabs */
div[data-testid="stTabs"]{border-bottom:1px solid var(--border)!important;margin-bottom:1.8rem!important;}
button[role="tab"]{background:transparent!important;color:var(--txt2)!important;border:none!important;border-bottom:2px solid transparent!important;border-radius:0!important;font-family:'Inter',sans-serif!important;font-weight:600!important;font-size:0.88rem!important;padding:0.65rem 1.3rem!important;transition:all 0.18s ease!important;}
button[role="tab"]:hover{color:var(--txt)!important;}
button[role="tab"][aria-selected="true"]{color:var(--accentL)!important;border-bottom:2px solid var(--accent)!important;background:transparent!important;}
div[role="tabpanel"]{padding-top:0.5rem!important;}

/* Inputs */
div[data-testid="stTextInput"] input,div[data-testid="stTextArea"] textarea{background:var(--bg4)!important;border:1px solid var(--border)!important;border-radius:10px!important;color:var(--txt)!important;font-family:'Inter',sans-serif!important;font-size:0.95rem!important;}
div[data-testid="stTextInput"] input:focus,div[data-testid="stTextArea"] textarea:focus{border-color:var(--accent)!important;box-shadow:0 0 0 3px rgba(99,102,241,0.18)!important;outline:none!important;}
div[data-testid="stTextInput"] input::placeholder,div[data-testid="stTextArea"] textarea::placeholder{color:#4a6080!important;}

/* Buttons */
div[data-testid="stButton"]>button{background:linear-gradient(135deg,#6366f1,#818cf8)!important;color:white!important;border:none!important;border-radius:10px!important;font-family:'Inter',sans-serif!important;font-weight:600!important;font-size:0.88rem!important;padding:0.52rem 1.4rem!important;transition:all 0.2s ease!important;box-shadow:0 4px 12px rgba(99,102,241,0.3)!important;letter-spacing:0.01em!important;}
div[data-testid="stButton"]>button:hover{transform:translateY(-1px)!important;box-shadow:0 6px 20px rgba(99,102,241,0.45)!important;}
div[data-testid="stButton"]>button[kind="secondary"]{background:var(--bg3)!important;box-shadow:none!important;border:1px solid var(--border)!important;color:var(--txt2)!important;}
div[data-testid="stButton"]>button[kind="secondary"]:hover{transform:none!important;border-color:var(--accent2)!important;color:var(--txt)!important;}

/* Quick-access chips */
div[data-testid="stHorizontalBlock"]>div[data-testid="column"]>div[data-testid="stButton"]>button{background:var(--bg3)!important;border:1px solid var(--borderL)!important;border-radius:20px!important;color:var(--accentL)!important;font-weight:700!important;font-size:0.78rem!important;padding:4px 12px!important;box-shadow:none!important;letter-spacing:0.03em!important;}
div[data-testid="stHorizontalBlock"]>div[data-testid="column"]>div[data-testid="stButton"]>button:hover{background:var(--bg4)!important;border-color:var(--accent)!important;color:var(--txt)!important;transform:none!important;box-shadow:none!important;}

/* Period radio */
div[data-testid="stRadio"]>label{color:var(--txt2)!important;font-size:0.78rem!important;font-weight:600!important;letter-spacing:0.05em!important;text-transform:uppercase!important;}
...
div[data-testid="stRadio"]>div>label{background:var(--bg3)!important;border:1px solid var(--borderL)!important;border-radius:20px!important;padding:4px 13px!important;cursor:pointer!important;transition:all 0.15s ease!important;color:var(--txt2)!important;font-weight:500!important;font-size:0.8rem!important;}
...
div[data-testid="stRadio"]>div>label:has(input:checked){background:linear-gradient(135deg,rgba(99,102,241,0.3),rgba(129,140,248,0.2))!important;border-color:var(--accent2)!important;color:var(--accentL)!important;font-weight:700!important;}
...
def build_chart(ticker, period):
    ...
    for color,fill in [("rgba(0,0,0,0)",fc2),("rgba(0,0,0,0)",fc)]:
        fig.add_trace(go.Scatter(x=dates,y=prices,mode="lines",line=dict(color=color,width=0),
...
# ══════════════════  HELPER: safe dividend yield  ════════════════════════════

def safe_div_yield(info):
    """Returns display string like '1.52%' or None if no dividend."""
    for key in ("dividendYield","trailingAnnualDividendYield"):
        val = info.get(key)
        if val and isinstance(val, (int, float)) and val > 0:
            # yfinance returns as decimal (e.g. 0.0152 = 1.52%)
            # Guard: if someone passes an already-pct value > 1, cap it
            pct = val * 100 if val < 1 else val
            if 0.01 < pct < 30:   # sanity check: ignore garbage values
                return f"{pct:.2f}%"
    return None
...
def html_stock_overview(ticker, info):
    """Compact overview card shown at top of stock analysis."""
    ...
    if bio:
        ...
        if not short_bio.endswith("."): short_bio += "."
    ...
    if rec_key and analysts:
        rec_colors = {
            "Strong Buy":"#34d399","Buy":"#34d399","Overweight":"#34d399",
            "Hold":"#fbbf24","Neutral":"#fbbf24","Equal Weight":"#fbbf24",
            "Underperform":"#f87171","Sell":"#f87171","Strong Sell":"#f87171"
        }
        rc = rec_colors.get(rec_key, "#8da4c4")
        target_line = ""
        if target and price:
            upside = (target - price) / price * 100
            upside_s = f"+{upside:.1f}%" if upside >= 0 else f"{upside:.1f}%"
            upside_c = "#34d399" if upside >= 0 else "#f87171"
            target_line = f'<div style="font-size:0.78rem;color:#8da4c4;margin-top:2px">Target <strong style="color:#eef2ff">${target:.0f}</strong> · <span style="color:{upside_c}">{upside_s} upside</span></div>'
        analyst_html = f"""
<div style="background:#1a2f50;border:1px solid rgba(99,102,241,0.15);border-radius:12px;padding:12px 16px;text-align:center;min-width:140px">
  <div style="font-size:0.68rem;color:#8da4c4;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:4px">Analyst Consensus</div>
  <div style="font-size:0.92rem;font-weight:800;color:{rc};margin-bottom:2px">{rec_key}</div>
  <div style="font-size:0.72rem;color:#8da4c4">{analysts} analysts</div>
  {target_line}
</div>"""
    ...
    if revenue:
        ...
        rev_html = f"""
<div style="background:#1a2f50;border:1px solid rgba(99,102,241,0.15);border-radius:12px;padding:12px 16px;text-align:center;min-width:110px">
  <div style="font-size:0.68rem;color:#8da4c4;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:4px">Annual Revenue</div>
  <div style="font-size:1.0rem;font-weight:800;color:#eef2ff">{rev_s}</div>
</div>"""

    right_blocks = analyst_html + rev_html
    right_section = f'<div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-start">{right_blocks}</div>' if right_blocks else ""
...
<div style="background:#112038;border:1px solid rgba(99,102,241,0.18);border-radius:16px;
  padding:1.3rem 1.6rem;margin-bottom:1rem;box-shadow:0 4px 20px rgba(0,0,0,0.2)">
    ...
    <div style="text-align:right">
      ...
      <div style="margin-top:7px">
        <span style="font-size:0.85rem;font-weight:700;color:{cc};background:{cbg};padding:4px 12px;border-radius:20px">
          {arr} {sgn}{day_chg:.2f} ({sgn}{day_pct:.2f}%)</span></div>
...
def html_news_grid(items, summary):
    """Refreshed card-grid news layout."""
    if not items:
        return ""
    ...
    if summary:
        sum_block=f"""
<div style="display:flex;gap:12px;align-items:flex-start;background:linear-gradient(135deg,rgba(99,102,241,0.1),rgba(129,140,248,0.06));
  ...
  <div>
    <div style="font-size:0.7rem;font-weight:700;color:#818cf8;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:4px">AI News Summary</div>
    <div style="font-size:0.87rem;line-height:1.7;color:#eef2ff">{summary}</div>
  </div>
</div>"""

    # Source → color mapping for badges
    source_colors = {
        "Reuters":"#fb923c","Bloomberg":"#6366f1","CNBC":"#34d399",
        "MarketWatch":"#818cf8","Yahoo Finance":"#a78bfa",
        "Seeking Alpha":"#f59e0b","Motley Fool":"#34d399",
    ...
    for i, item in enumerate(items[:6]):
        ...
        date    = item.get("date","")
        sc      = source_colors.get(pub, "#8da4c4")
        pub_badge= f'<span style="font-size:0.7rem;font-weight:700;color:{sc};background:{sc}18;padding:2px 8px;border-radius:10px;white-space:nowrap">{pub}</span>' if pub else ""
        date_tag = f'<span style="font-size:0.7rem;color:#4a6080">{date}</span>' if date else ""
        meta_row = " · ".join(p for p in [pub_badge, date_tag] if p)

        cards+=f"""
<a href="{link}" target="_blank" style="text-decoration:none;display:block">
  <div style="background:#1a2f50;border:1px solid rgba(99,102,241,0.12);border-radius:13px;
    padding:14px 16px;height:100%;transition:border-color 0.15s;
    display:flex;flex-direction:column;justify-content:space-between;gap:10px">
    <div style="font-size:0.87rem;font-weight:600;color:#eef2ff;line-height:1.5;
      display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden">{title}</div>
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">{meta_row}</div>
...
<div style="margin-bottom:10px">
  ...
  <div style="background:#1a2f50;border-radius:4px;height:6px;overflow:hidden">
    ...
    return f"""
<div style="background:#112038;border:1px solid rgba(99,102,241,0.18);border-radius:16px;padding:1.4rem 1.6rem;margin-bottom:1rem;box-shadow:0 4px 20px rgba(0,0,0,0.2)">
  ...
  <div style="display:grid;grid-template-columns:auto 1fr;gap:24px;align-items:center">
    <div style="text-align:center;padding:0 8px">
      ...
      <div style="font-size:1.1rem;font-weight:700;color:{color};background:{gbg};padding:3px 16px;border-radius:20px;display:inline-block">{grade}</div>
    </div>
    <div>{bars}</div>
  </div>
</div>"""
...
<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid rgba(99,102,241,0.07)">
  ...
  <div style="flex:1;min-width:0">
    ...
    <div style="font-size:0.72rem;color:#8da4c4;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{(info_map.get(t,{}).get('longName') or info_map.get(t,{}).get('shortName') or t)[:28]}</div>
  </div>
  <div style="text-align:right">
    <div style="font-size:0.85rem;font-weight:700;color:#eef2ff">{w/total_w*100:.0f}%</div>
    <div style="font-size:0.68rem;color:{SECTOR_COLORS.get(info_map.get(t,{}).get('sector') or ('ETF' if (info_map.get(t,{}).get('quoteType') or '').upper() in ('ETF','MUTUALFUND') else 'Unknown'),'#475569')};background:rgba(0,0,0,0.2);padding:1px 6px;border-radius:8px;margin-top:2px">{(info_map.get(t,{}).get('sector') or ('ETF' if (info_map.get(t,{}).get('quoteType') or '').upper() in ('ETF','MUTUALFUND') else 'Unknown'))[:14]}</div>
  </div>
</div>""" for t,w in tw.items())
    return f"""
<div style="background:#112038;border:1px solid rgba(99,102,241,0.18);border-radius:16px;padding:1.2rem 1.5rem;box-shadow:0 4px 20px rgba(0,0,0,0.2)">
  <div style="font-size:0.73rem;font-weight:700;color:#8da4c4;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:10px">Your Holdings</div>
  {rows}<div style="padding-top:4px"></div>
</div>"""

def html_recs(recs_list,raw_text):
    if not recs_list: return f'<div style="font-size:0.87rem;color:#8da4c4;line-height:1.7">{raw_text}</div>'
    return "".join(f"""
<div style="background:#1a2f50;border:1px solid rgba(99,102,241,0.15);border-radius:12px;padding:13px 16px;margin-bottom:9px;display:flex;gap:14px;align-items:flex-start">
...
def html_stock_pick_card(pick,pick_info):
    ticker=pick.get("ticker","—"); name=pick.get("name",ticker)
    sector=pick.get("sector","—"); horizon=pick.get("horizon","—")
    rating=pick.get("rating","Buy"); tagline=pick.get("tagline",""); thesis=pick.get("thesis","")
    ...
    return f"""
<div style="background:linear-gradient(135deg,#112038,#142540);border:1px solid rgba(99,102,241,0.25);border-radius:20px;padding:2rem;margin-bottom:1.2rem;box-shadow:0 6px 32px rgba(0,0,0,0.3)">
...
with c_logo:
    st.markdown("""
<div style="display:flex;align-items:center;gap:12px;padding-bottom:1.3rem;border-bottom:1px solid rgba(99,102,241,0.15);margin-bottom:1.4rem">
...
if not ticker:
    ...
    with tab_overview:
        ...
        if gainers or losers:
            ...
            st.info("Market data unavailable. Markets may be closed or rate-limited.")
...
<div style="margin-bottom:0.8rem">
  ...
  <div style="font-size:0.82rem;color:#8da4c4;margin-top:3px">An AI-curated investment idea with thesis, catalysts, and risk analysis</div>
</div>""", unsafe_allow_html=True)
        with hc2:
        ...
        if not has_pick:
            ...
            if no_key:
                st.info("⚠️ Add your Anthropic API key to Streamlit secrets to enable this feature.")
            else:
                ...
                if raw:
                    ...
                    st.error("Couldn't generate a pick right now. Try again shortly.")
        ...
        if has_pick and st.session_state.stock_pick:
            ...
            if pt:
                ...
                if st.button(f"🔍 Full Analysis for {pt} →",key="pick_goto"):
                    st.session_state.ticker=pt; st.session_state.ticker_query=pt
                    st.session_state.company_name=pick.get("name",""); st.rerun()
    ...
    # ── Tab 3: Portfolio ──────────────────────────────────────────────────────
    with tab_portfolio:
        st.markdown(html_section_title("💼","Portfolio Analyzer","Score your diversification + get AI recommendations"), unsafe_allow_html=True)
        with st.container(border=True):
            st.markdown("""
<div style="font-size:0.84rem;color:#8da4c4;line-height:1.65;margin-bottom:0.9rem">
  Enter tickers separated by commas. <span style="color:#818cf8">Add <code style="background:#1a2f50;padding:1px 6px;border-radius:4px;font-size:0.8rem">:weight</code> for allocation.</span><br>
  <span style="font-size:0.77rem;opacity:0.8">e.g. <code style="background:#1a2f50;padding:1px 6px;border-radius:4px">AAPL, TSLA, MSFT</code> or <code style="background:#1a2f50;padding:1px 6px;border-radius:4px">AAPL:40, VTI:30, BND:20, GLD:10</code></span>
</div>""", unsafe_allow_html=True)
            port_raw=st.text_area("holdings",placeholder="AAPL, MSFT, NVDA, VTI:40, BND:20...",label_visibility="collapsed",height=76,key="port_input")
        ...
        if clr_port:
            st.session_state.show_port_result=False; st.session_state.port_result=None; st.rerun()
        if run_port and port_raw.strip():
            tw=parse_portfolio(port_raw)
            if not tw: st.warning("Please enter at least one ticker.")
            else:
        ...
        if st.session_state.show_port_result and st.session_state.port_result:
            ...
            with cd:
                ...
                st.plotly_chart(sector_donut(pr["score"]["sectors"]),use_container_width=True,config={"displayModeBar":False})
                st.markdown('</div>', unsafe_allow_html=True)
            with ch:
                st.markdown(html_holdings(pr["tw"],pr["info_map"]), unsafe_allow_html=True)
            st.markdown('<div style="background:#112038;border:1px solid rgba(99,102,241,0.18);border-radius:16px;padding:1.4rem 1.6rem;margin-top:0.4rem;box-shadow:0 4px 20px rgba(0,0,0,0.2)"><div style="font-size:1.0rem;font-weight:800;color:#eef2ff;margin-bottom:6px">🤖 AI Recommendations</div><div style="font-size:0.8rem;color:#8da4c4;margin-bottom:1rem">Specific additions to improve your diversification:</div>', unsafe_allow_html=True)
            st.markdown(html_recs(pr["recs"],pr["raw_recs"]), unsafe_allow_html=True)
            st.markdown('<div style="font-size:0.7rem;color:#3a5070;margin-top:0.8rem">⚠️ For informational purposes only. Not financial advice.</div></div>', unsafe_allow_html=True)
...
# ══════════════════  STOCK ANALYSIS  ══════════════════════════════════════════
    ...
    # ② Company at a Glance (NEW — brief overview)
    st.markdown(html_stock_overview(ticker, info), unsafe_allow_html=True)

    # ③ Key stats strip (with fixed dividend yield)
