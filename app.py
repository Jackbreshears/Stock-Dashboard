...
.resolved-name { color: #e6edf3; font-weight: 600; }
.resolved-tick { color: #58a6ff; font-weight: 700; }
...
.mini-metric { background: #0d1117; border-radius: 8px; padding: 8px 12px; margin-bottom: 6px; }
...
.consensus-badge { background: #0d1117; border-radius: 8px; text-align: center; padding: 14px; margin-bottom: 10px; }
.c-sm { font-size: 11px; color: #484f58; }
.c-big { font-size: 24px; font-weight: 700; }
.c-n { font-size: 11px; color: #484f58; margin-top: 4px; }
.upside-block { background: #0d1117; border-radius: 8px; text-align: center; padding: 12px; margin-bottom: 10px; }
.upside-pct { font-size: 30px; font-weight: 800; }
.upside-sub { font-size: 11px; color: #484f58; margin-top: 2px; }
.t-row { display: flex; justify-content: space-between; padding: 5px 0; border-bottom: 1px solid #21262d; font-size: 13px; }
.t-lbl { color: #8b949e; } .t-val { font-weight: 700; color: #e6edf3; }
.gauge-badge { background: #0d1117; border-radius: 10px; text-align: center; padding: 16px; margin-bottom: 10px; }
.g-icon { font-size: 42px; } .g-lbl { font-size: 21px; font-weight: 700; margin-top: 4px; }
.g-plain { font-size: 12px; color: #484f58; margin-top: 4px; }
.sig-why { font-size: 12px; font-weight: 700; color: #8b949e; margin: 10px 0 6px; }
.sig-item { background: #0d1117; border-radius: 6px; font-size: 12px; padding: 7px 10px; margin-bottom: 4px; color: #e6edf3; }
.stat-cell { background: #0d1117; border-radius: 8px; padding: 9px 12px; margin-bottom: 6px; }
.stat-top { display: flex; justify-content: space-between; align-items: center; }
.stat-lbl { font-size: 11px; color: #484f58; } .stat-val { font-size: 14px; font-weight: 700; }
.stat-exp { font-size: 11px; color: #484f58; margin-top: 3px; line-height: 1.4; }
.pros-box { background: #0a1f14; border: 1px solid #3fb95033; border-radius: 8px; padding: 14px; }
.cons-box { background: #1f0a0e; border: 1px solid #f8514933; border-radius: 8px; padding: 14px; }
...
.hl-item { background: #0d1117; border-radius: 6px; padding: 10px 14px; margin-bottom: 6px; }
...
# ══════════════════════════════════════════════════════════════════════════════
#  Ticker resolver — accepts company names OR ticker symbols
# ══════════════════════════════════════════════════════════════════════════════

@st.cache_data(ttl=3600, show_spinner=False)
def resolve_ticker(query: str) -> tuple:
    """
    Given a user query (ticker symbol OR company name), return (ticker, display_name).
    Returns ("", "") if nothing found.

    Strategy:
      1. If query looks like a ticker (short, no spaces), try it directly first.
      2. Use yf.Search() to find the best matching equity/ETF.
      3. Fall back to treating the raw query as a ticker.
    """
    query = query.strip()
    if not query:
        return ("", "")
    ...
    # Step 1: try directly as a ticker if it looks like one
    ...
    # Step 2: search by name or partial ticker
    try:
        results = yf.Search(query, max_results=8)
        quotes  = getattr(results, "quotes", []) or []
        preferred_types = {"EQUITY", "ETF"}
        equities = [q for q in quotes if q.get("quoteType", "").upper() in preferred_types]
        if not equities:
            equities = quotes
        if equities:
            best   = equities[0]
    ...
    # Step 3: last resort — treat raw input as ticker
    try:
        ticker = query.upper()
        ...
        for n in raw_items[:6]:
            if "content" in n and isinstance(n["content"], dict):
                ...
                try:
                    ...
                    ).strftime("%b %d, %Y") if raw_date else ""
                except Exception:
                    date_str = ""
            else:
                title    = n.get("title", "")
...
def build_chart(ticker: str, period: str):
    ...
    fig = make_subplots(rows=2 if has_vol else 1, cols=1, shared_xaxes=True,
                        row_heights=[0.76, 0.24] if has_vol else [1], vertical_spacing=0.04)
    fig.add_trace(go.Scatter(x=dates, y=prices, mode="lines",
                             line=dict(color=lcolor, width=2), fill="tozeroy", fillcolor=fcolor,
                             name="Price", hovertemplate="$%{y:.2f}<extra></extra>"), row=1, col=1)
    fig.add_hline(y=float(prices.iloc[0]), line_dash="dot", line_color="#30363d", line_width=1, row=1, col=1)
    if has_vol:
        vol_colors = ["#3fb950" if float(prices.iloc[i]) >= (float(prices.iloc[i-1]) if i > 0 else float(prices.iloc[0]))
                      else "#f85149" for i in range(len(prices))]
        fig.add_trace(go.Bar(x=dates, y=volumes, marker_color=vol_colors, marker_opacity=0.45,
                             name="Volume", hovertemplate="%{y:,.0f}<extra>Volume</extra>"), row=2, col=1)
    arrow = "▲" if is_up else "▼"
    fig.update_layout(
        title=dict(text=f"<b>{ticker}</b>   <span style='color:{lcolor}'>{arrow} {pct_chg:+.2f}% over this period</span>",
                   font=dict(size=13), x=0, xanchor="left"),
        paper_bgcolor="#161b22", plot_bgcolor="#0d1117", font=dict(color="#8b949e", size=11),
        margin=dict(l=10, r=60, t=44, b=10), hovermode="x unified", showlegend=False,
        height=340 if has_vol else 270,
        xaxis=dict(showgrid=False, zeroline=False),
        yaxis=dict(showgrid=True, gridcolor="#21262d", zeroline=False, tickprefix="$", side="right"),
    )
    if has_vol:
...
def render_analyst(analyst: dict) -> None:
    ...
    st.markdown(f'<div class="consensus-badge" style="border:2px solid {color};">'
    ...
    if tm and cp:
        ...
        arrow = "▲" if pct >= 0 else "▼"
        st.markdown(f'<div class="upside-block"><div class="c-sm">Potential Upside</div>'
                    f'<div class="upside-pct" style="color:{col};">{arrow} {abs(pct):.1f}%</div>'
                    f'<div class="upside-sub">Analysts avg target: ${tm:.2f}</div></div>',
                    unsafe_allow_html=True)
    tl, th = analyst.get("target_low"), analyst.get("target_high")
    if tl and th and tm:
        for lbl, val in [("🎯 Average target", f"${tm:.2f}"), ("⬇️  Pessimistic", f"${tl:.2f}"), ("⬆️  Optimistic", f"${th:.2f}")]:
...
def render_sentiment(sentiment: dict) -> None:
    ...
    plain = {"Bullish": "Most signs point upward 📈", "Neutral": "Mixed signals — watch and wait 👀",
             "Bearish": "Most signs point downward 📉"}.get(label, "")
    st.markdown(f'<div class="gauge-badge" style="border:2px solid {color};">'
                f'<div class="g-icon">{icon}</div><div class="g-lbl" style="color:{color};">{label}</div>'
                f'<div class="g-plain">{plain}</div></div>', unsafe_allow_html=True)
    bar_pct = int((max(-5, min(5, score)) + 5) / 10 * 100)
    st.markdown(f'<div style="font-size:11px;color:#484f58;margin-bottom:4px;">Mood Meter</div>'
                f'<div style="background:#30363d;border-radius:4px;height:10px;margin-bottom:5px;">'
                f'<div style="width:{bar_pct}%;background:{color};height:10px;border-radius:4px;"></div></div>'
                f'<div style="display:flex;justify-content:space-between;font-size:10px;">'
                f'<span style="color:#f85149;">😨 Bearish</span>'
                f'<span style="color:#d29922;">😐 Neutral</span>'
                f'<span style="color:#3fb950;">😀 Bullish</span></div>', unsafe_allow_html=True)
...
def render_pros_cons(text: str) -> None:
    ...
    c1, c2 = st.columns(2)
    with c1:
        items = "".join(f'<div class="pc-item">• {it}</div>' for it in pros) or '<div class="pc-item">—</div>'
        st.markdown(f'<div class="pros-box"><div class="pc-hdr" style="color:#3fb950;">✅&nbsp; Reasons to consider buying</div>{items}</div>',
                    unsafe_allow_html=True)
    with c2:
        items = "".join(f'<div class="pc-item">• {it}</div>' for it in cons) or '<div class="pc-item">—</div>'
        st.markdown(f'<div class="cons-box"><div class="pc-hdr" style="color:#f85149;">⚠️&nbsp; Risks to be aware of</div>{items}</div>',
                    unsafe_allow_html=True)
...
def main() -> None:
    # Session state persists the resolved ticker across widget reruns
    ...
    st.markdown('<div class="app-header">'
                ...
                '<div class="app-sub">Search by company name or ticker &nbsp;•&nbsp; No finance experience needed</div>'
                '</div>', unsafe_allow_html=True)
    ...
    with col_in:
        query_input = st.text_input(
            "Search",
            placeholder="Company name or ticker — e.g. Apple, Tesla, NVDA, Microsoft…",
    ...
    # Resolve ticker when Analyze is clicked
    if analyze and query_input.strip():
        ...
        if resolved_ticker:
            ...
            st.error(f"❌  Couldn't find a stock matching '{query_input}'. "
                     "Try using the official ticker symbol (e.g. AAPL for Apple).")
    ...
    if not ticker:
        ...
        st.markdown('<div style="text-align:center;padding:60px 0;">'
                    ...
                    'Search for any company above and click <strong>Analyze</strong></div>'
                    '<div style="font-size:14px;color:#e6edf3;margin-bottom:8px;">'
                    'You can search by company name <em>or</em> ticker symbol</div>'
                    '<div style="font-size:13px;color:#484f58;">'
                    'Apple &nbsp;•&nbsp; Tesla &nbsp;•&nbsp; Nvidia &nbsp;•&nbsp; Microsoft &nbsp;•&nbsp; Amazon<br/>'
    ...
    # "Analyzing:" badge — confirms what company was matched
    company_name = price_data.get("name", ticker)
    st.markdown(
        f'<div class="resolved-badge">'
        f'📌 Analyzing: &nbsp;<span class="resolved-name">{company_name}</span>'
    ...
    # Footer
    st.markdown("<br/>", unsafe_allow_html=True)
    st.markdown(f'<div class="footer">Data provided by Yahoo Finance via yfinance &nbsp;•&nbsp; '
                f'AI analysis by {CLAUDE_MODEL} &nbsp;•&nbsp; '
                f'Not financial advice — do your own research before investing.</div>',
                unsafe_allow_html=True)
