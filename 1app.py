...
[data-testid="stButton"]>button{
  ...
  font-weight:600!important;padding:10px 20px!important;cursor:pointer!important;
...
.chip-row [data-testid="stButton"]>button{
  ...
  color:var(--accent2)!important;font-size:12px!important;padding:6px 10px!important;
...
details[open] summary span.arr{transform:rotate(90deg);display:inline-block}
</style>
...
def fetch_movers():
    ...
    for t in tickers:
        try:
            ...
            chg=getattr(inf,"percent_change",None); price=getattr(inf,"last_price",None)
...
def html_stat_card(label,value,explainer=None,icon=""):
    exp=""
    if explainer:
        exp=f'<div style="color:#475569;font-size:11px;margin-top:5px;line-height:1.3">{explainer}</div>'
    return (
        '<div style="background:#1e2438;border:1px solid #2e3650;border-radius:12px;padding:16px 18px;height:100%">'
...
def html_stock_overview(ticker,info):
    ...
    emp_html=(
        f'<span style="background:#1e3a5f;color:#60a5fa;padding:3px 9px;border-radius:20px;font-size:12px;margin-left:8px">'
        f'{emp:,} employees</span>' if emp else ""
    )
    tags=""
    for lbl,val in [("Sector:",sector),("Industry:",industry),("Country:",country)]:
        if val:
    ...
    rev_block="" if not rev or rev=="N/A" else (
        f'<div style="background:#242b3d;border-radius:10px;padding:10px 16px">'
        f'<div style="color:#64748b;font-size:11px;margin-bottom:3px">ANNUAL REVENUE</div>'
        f'<div style="color:#e2e8f0;font-weight:700">{rev}</div></div>'
    )
    rec_block="" if not rec else (
        f'<div style="background:#242b3d;border-radius:10px;padding:10px 16px">'
        f'<div style="color:#64748b;font-size:11px;margin-bottom:3px">ANALYST CONSENSUS</div>'
        f'<div style="color:{r_color};font-weight:700">{rec}{upside}</div></div>'
    )
    return (
...
def html_news_card(item):
    ...
    s_dot,s_label,s_color=sent_map[sent]
    pub_colors={"Reuters":"#f87171","Bloomberg":"#fbbf24","CNBC":"#f97316","WSJ":"#60a5fa"}
    pub_color=pub_colors.get(pub,"#94a3b8")
    pub_html=(
        f'<span style="color:{pub_color};font-size:11px;font-weight:700;background:{pub_color}22;'
    ...
    return (
        ...
        'padding:16px 18px;margin-bottom:10px;display:block">'
        + f'<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap">'
        + pub_html
        + f'<span style="color:{s_color};font-size:11px;font-weight:600;'
        + f'background:{s_color}22;padding:2px 8px;border-radius:20px">{s_dot} {s_label}</span>'
        ...
        + '<div style="color:#6366f1;font-size:12px;margin-top:8px;font-weight:500">Read &rarr;</div>'
        + '</div></a>'
    )
...
def html_portfolio_full(analysis,tw,info_map,score):
    ...
    cfg={"KEEP":("#0f2d1a","#34d399","#166534","KEEP"),"REDUCE":("#2d0f0f","#f87171","#7f1d1d","REDUCE")}
    cards=""
    for h in holdings:
        ...
        cards += (
            ...
            + f'<span style="background:{tag_bg};color:{border};font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px">{tag_label}</span></div>'
            + f'<div style="text-align:right"><div style="color:#e2e8f0;font-weight:600">${price:,.2f} x {shares:.0f} shares</div>'
    ...
    add_sec=(
        '<div style="margin:16px 0 6px;color:#94a3b8;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Suggested Additions</div>'
        + add_cards
    ) if add_cards else ""
    return (
        ...
        + f'<div style="color:{sc};font-size:28px;font-weight:800">{score}</div>'
        + f'<div style="color:{sc};font-size:12px">/100 &middot; {sl}</div></div>'
        + f'<div style="text-align:center;background:#242b3d;border-radius:10px;padding:12px 20px">'
...
def render_sidebar():
    with st.sidebar:
        ...
        if wl:
            ...
            st.caption("No stocks saved. Tap 'Add to Watchlist' on any stock page.")
        st.markdown("---")
...
GLOSSARY = [
    ("Stock",
     "A tiny piece of ownership in a company. If the company does well, your piece (called a share) is worth more. If it does badly, it is worth less."),
    ("ETF - Exchange Traded Fund",
     "A bundle of many stocks sold as one. Like buying a fruit basket instead of one fruit — more variety, less risk from any single company."),
    ("P/E Ratio (Price-to-Earnings)",
     "If a stock has a P/E of 20, you are paying $20 for every $1 the company earns per year. Lower can mean cheaper, but context matters."),
    ("Beta",
     "Measures how bumpy the ride is. A beta above 1.0 means the stock moves more than the market. Below 1.0 means calmer. Above 2.0 means wild."),
    ("Dividend",
     "Some companies pay you cash just for owning their stock — like collecting rent. A 3% dividend yield means $30/year on a $1,000 investment."),
    ("Diversification",
     "Do not put all your eggs in one basket. Spreading money across different companies and sectors means one bad stock cannot wipe you out."),
    ("Bull vs Bear Market",
     "A bull market means prices are rising overall (the bull charges upward). A bear market means prices are falling (the bear swipes downward)."),
    ("Market Cap",
     "The total value of a company. Calculated as share price times total shares. Apple is ~$3 trillion. A small startup might be $50 million."),
    ("Index Fund",
     "A fund that automatically copies a market index like the S&P 500. You instantly own tiny pieces of 500 big US companies for very low fees."),
    ("Portfolio",
     "Your personal collection of all your investments. A balanced portfolio mixes different types — stocks, ETFs, bonds — to spread out risk."),
    ("Earnings Report",
     "Every 3 months (a quarter), companies report how much money they made. A good surprise usually sends the stock up. A bad one sends it down."),
    ("Compound Returns",
     "Earning returns on your returns. $1,000 growing at 10% per year becomes $17,449 after 30 years. The longer you wait, the more powerful it gets."),
]
...
def render_stock_page(ticker):
    ...
    with c1:
        st.markdown(
            ...
            f'<div style="font-size:26px;font-weight:800;color:#e2e8f0">{name}</div>'
            f'<div style="color:#94a3b8;font-size:14px">{ticker} &middot; {info.get("exchange","")}</div></div>',
            unsafe_allow_html=True)
    with c2:
        st.markdown(
            f'<div style="padding:8px 0">'
            f'<div style="font-size:34px;font-weight:800;color:#e2e8f0">${price:,.2f}</div>'
            f'<div style="color:{price_color};font-size:16px;font-weight:600">{sign}${abs(chg):.2f} ({chg_pct:+.2f}%)</div></div>',
            unsafe_allow_html=True)
    with c3:
...
def render_home():
    ...
    with tab1:
        ...
        if st.session_state.movers_loaded:
                ...
                for i,m in enumerate(movers):
                    with cols[i%4]:
                        ...
                        st.markdown(
                            ...
                            f'<div style="color:{c};font-weight:600;font-size:15px">{sign}{abs(m["chg"]):.2f}%</div></div>',
                            unsafe_allow_html=True)
            ...
            st.markdown(
                ...
                '<div style="color:#475569;font-size:12px;margin-top:8px">Live data from Yahoo Finance</div></div>',
                unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if pick:
            ...
            cats="".join(
                f'<div style="color:#e2e8f0;font-size:13px;margin-bottom:4px">&rarr; {p.get(f"CATALYST{i}","")}</div>'
                for i in range(1,4) if p.get(f"CATALYST{i}"))
            risks="".join(
                f'<div style="color:#f87171;font-size:13px;margin-bottom:4px">&#9888; {p.get(f"RISK{i}","")}</div>'
    ...
    with tab3:
        ...
        st.markdown(
            ...
            'what to <span style="color:#34d399;font-weight:600">KEEP</span>, '
            'what to <span style="color:#f87171;font-weight:600">REDUCE</span>, and '
            'what to <span style="color:#93c5fd;font-weight:600">ADD</span> to balance your portfolio.'
            '</div></div>',
            unsafe_allow_html=True)
        with st.container(border=True):
            ...
            if tw:
                with st.spinner("Fetching data and running analysis..."):
                    ...
                    if raw:
                        ...
                        st.session_state.port_result={"analysis":analysis,"tw":tw,"info_map":info_map,
    ...
    with tab4:
        ...
        st.write("New to investing? These 12 concepts cover 90% of what you need to know. Click any term to expand it.")

        # Use HTML <details>/<summary> instead of st.expander to avoid the _arrow_right rendering bug
        cards_html = ""
        for term, definition in GLOSSARY:
            cards_html += (
                f'<details style="background:#1e2438;border:1px solid #2e3650;border-radius:10px;'
                f'padding:0;margin-bottom:8px;overflow:hidden">'
                f'<summary style="padding:14px 18px;cursor:pointer;font-weight:600;font-size:14px;'
                f'color:#e2e8f0;list-style:none;user-select:none">'
                f'<span style="color:#6366f1;margin-right:8px">&#9654;</span>{term}</summary>'
                f'<div style="padding:4px 18px 16px 44px;color:#94a3b8;font-size:14px;line-height:1.7">'
        ...
        st.write("Test your knowledge — pick the correct answer:")

        with st.container(border=True):
            st.markdown("**What does a P/E ratio of 30 mean?**")
            a1=st.radio("q1", [
                "The stock has 30% annual returns",
            ...
            if a1 is not None:
                if "pay $30" in a1:
                    st.success("Correct! P/E = the price you pay per $1 of company earnings.")
                else:
        ...
        with st.container(border=True):
            ...
            if a2 is not None:
                if "spreading" in a2.lower():
                    st.success("Correct! Diversification is the #1 way to reduce risk.")
                else:
                    st.error("Not quite. Spreading money across different sectors (diversification) is the answer.")
